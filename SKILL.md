...
> Reading intel is always on you — no hook reads a note for you. With the reflexes pack, the intel-cue names notes whose `glob:` matches a file you're editing — a reminder, not a substitute; the glance and the judgment stay yours. So in an Echo project, err toward looking: glance at the area the first time you touch it.
...
- **Scope `glob:` tightly.** It's a precision *hint* for when a note is relevant — the files the fact bears on — that you use when reading an area to judge whether a note touches the file in front of you. Match only where it applies; a glob covering half the repo guides nothing — and with the reflexes pack's intel-cue firing on it mechanically, an over-broad glob cries wolf on every edit.
