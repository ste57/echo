...
2. **Judge.** Would the next session get this wrong without it, *and* is it still true next week? Either no → stop. Recoverable by reading the code → fails. Narrating the change you just made → fails (that's the PR description's job).
3. **Reconcile.** Search what's filed by *content*, not name: grep `intel/` for the fact's key terms — filenames and `when:` lines rarely reveal what a body holds. Covered → stop. Similar → update in place. **Contradicts → ask before overwriting. Never silently replace.**
4. **Save.** *Sure* (explicit teach, clear correction, firm rule, stated preference, confirmed gotcha) → save now, acknowledge in one line: *"noted: …"*. *Inferred* → don't interrupt; **propose it, batched, at the next natural stop** — a real breakpoint, because the pending batch dies with a compaction — and save what gets a yes.
