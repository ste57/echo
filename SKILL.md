...
If a single teach carries *both* a repeatable sequence and a durable fact, split it: the steps → a playbook, the fact → intel, and the playbook *references* the fact rather than restating it, so the fact keeps one home. One home cuts the other way too: a profile or intel capture that falls inside an existing playbook's domain is written as a *pointer* to the playbook plus the delta (a waiver, an exception) — never a paraphrase of its steps; a stripped summary answers confidently and wrongly.
...
4. **Write the activation block into `CLAUDE.md`** — a fenced pointer that boots Echo for *every* teammate, not just those who installed the skill (`CLAUDE.md` auto-loads for everyone). It's the one time Echo touches `CLAUDE.md`, and it's a pointer, never memory. Copy it verbatim from `reference/activation.md`.
